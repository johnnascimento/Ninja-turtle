...
let joker = Object.create(villan);
joker.init('Walter white, if you will', 'Joker');
cl(`${joker.name} is my name`);
cl(`${joker.changeCostume()} is not a real thing, is it?`);
...
// Making the variable that store the scope of this as being the spiderman object as its main reference instead of the window object
spiderman.allies = ['James Franco', 'Andrew Garfield', 'Gwen Stacy', 'Eddie Brook', 'The Iron Man', 'Captain America', 'Black widow' ];

spiderman.enemies = ['Shocker', 'Green imp', 'Rhyno', 'Electro', 'Venon', 'Carnage', 'Vulture'];

spiderman.findFriends = function(){
  that = this;
  this.allies.forEach(function(friend, idx) {
    console.log(`${friend} is friends with ${that.name}`);
  }
  );
}
spiderman.findFriends();

// Another way to do this is to use the bind method that is shared by all objects in js
spiderman.findEnemy = function(){
  this.enemies.forEach(function(enemy) {
    console.log(`${enemy} is completely against ${that.name}`);
  }.bind(this)
  );
}
